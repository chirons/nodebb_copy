...
function setup() {
	winston.info('通过命令行安装 NodeBB');

	var install = require('./src/install');
	var build = require('./build');

	process.stdout.write('\n欢迎来到 NodeBB!\n');
	process.stdout.write('\n这是一个全新的安装, 所以在安装过程中需要您选择您的环境相关的参数.\n');
	process.stdout.write('按 enter 接受 默认选择 (shown in brackets).\n');
	...
	async.series([
		...
		if (err) {
			winston.error('NodeBB 安装过程中发生错误: ', err.message);
		} else {
			if (data.hasOwnProperty('password')) {
				process.stdout.write('已经为您创建了一个管理员账户:\n');
				process.stdout.write('    管理员账号: ' + data.username + '\n');
				process.stdout.write('    密码: ' + data.password + '\n');
				process.stdout.write('\n');
			}
			process.stdout.write('NodeBB 安装成功. 运行 \'./nodebb start\' 命令手工启动服务.\n');
